...
# Builds DAS features for Human 1MB cloneset from Ensembl database
#
package Bio::Das::ProServer::SourceAdaptor::hs34_20_clone;
...
sub init {
  ...
  # +-----------------+-------------+---------+------+--------------------------------+
  my $query = qq(SELECT sr.name, sr.length, sr.seq_region_id as srid
...
sub build_entry_points {
  ...
  my @ep;
  foreach my $c ( $self->known_segments()) {
    push (@ep, {
		'segment'	=>	$c,
...
sub build_features_by_segment  {
  ...
  my $query = qq(SELECT mf.misc_feature_id as mfid, mf.seq_region_start as start,mf.seq_region_end as end ,mf.seq_region_strand as ori, ms.name as type
		 FROM misc_feature mf, seq_region sr, misc_feature_misc_set mfms, misc_set ms
		 WHERE mf.seq_region_end >= $start
		 AND mf.seq_region_start between ($start-2000000) and $end
  ...
  for my $row (@{$ref}) {
    ...
    my $mfid = $row->{'mfid'};
    my $query2 = qq(SELECT code, value
		    FROM misc_attrib ma, attrib_type at
		    WHERE misc_feature_id = '$mfid'
		    AND ma.attrib_type_id = at.attrib_type_id
		    AND at.code in ("name","non_ref","synonym"));

    my $ref2 = $self->transport->query($query2);
    my $name;
    for my $row2 (@{$ref2}) {
      if ($row2->{'code'} eq "name") {
	$name = $row2->{'value'};
	last;

      } elsif ($row2->{'code'} eq "non_ref") {
	$name = $row2->{'value'};
	last;

      } elsif ($row2->{'code'} eq "synonym") {
	$name = $row2->{'value'};
	last;

      } else {
	warn "Cannot map misc feature ID!\n";
      }
    }
...
sub build_types {
  ...
  foreach ("Tilepath", "1Mb cloneset", "32K cloneset") {
    push (@tmp,
	  {
	   'method'   => "ensembl",
	   'category' => "default",
